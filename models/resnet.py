...
class erfslLinear(nn.Module):
    def __init__(self, indim, outdim, weight=None):
        super(erfslLinear, self).__init__()
        self.L = nn.Linear(indim, outdim, bias = True)
        if weight is not None:
            self.L.weight.data = Variable(weight)

    def forward(self, x, selective_index=None):
        length = x.size(1)
        if selective_index is None:
            selective_index = np.arange(0, length, 1)
        selective_x = x[:, selective_index]
        selective_weight = self.L.weight[:, selective_index]
        scores = torch.mm(selective_x, selective_weight.transpose(0, 1))
        return scores
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias):
        ...
        self.erfsl_linear = erfslLinear(self.df, num_classes)
    ...
    def erfsl_forward(self, x, selective_index=None):
        out = self.fea(x)
        logits = self.erfsl_linear(out, selective_index=selective_index)
        return logits


def Reduced_ResNet18(nclasses, nf=64, bias=True):
